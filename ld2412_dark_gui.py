    ...
    def parse_command_response(self, frame):
        ...
        if command_code == 0x01A0:  # CMD_VERSION (回應碼)
            ...
            if len(frame) >= 20 and success:
                # 修正幀結構解析 - 門0敏感度在位置10，門1-13在位置11-23
                # 實際幀結構: 幀頭(4) + 長度(2) + 命令(2) + 狀態(1) + ACK狀態(1字節) + 門0敏感度(1字節) + 門1-13敏感度(13字節) + 幀尾(4)
                ...
                # 門敏感度數據解析：位置10=門0，位置11-23=門1-13
                sens_values = []
                # 門0敏感度在位置10
                sens_values.append(int(frame[10]))
                # 門1-13敏感度在位置11-23
                for i in range(11, min(24, len(frame)-4)):  # 排除幀尾4字節
                ...
                self.log(f"🔍 門0敏感度(位置10): {frame[10]:02X} = {frame[10]}")
                sens_hex = ' '.join([f'{frame[i]:02X}' for i in range(11, min(24, len(frame)-4))])
                self.log(f"🔍 門1-13敏感度(位置11-23): {sens_hex}")
